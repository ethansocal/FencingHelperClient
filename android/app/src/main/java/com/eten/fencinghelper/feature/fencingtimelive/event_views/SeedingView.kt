...
import com.eten.fencinghelper.modules.SeedingData
...
class SeedingViewModel(
    val apiService: ApiService,
    val event: TournamentData.Day.Event,
    val roundId: String
) :
    ViewModel() {
    private var _seedingData = MutableStateFlow<SeedingData?>(null)
    val seedingData = _seedingData.asStateFlow()
    ...
    private val _filtered = MutableStateFlow<List<SeedingData.Seeding>>(emptyList())
    ...
    fun onSearchTextChange(text: String) {
        ...
        _filtered.value = _seedingData.value?.seedings?.filter {
    ...
    suspend fun fetchData() {
        try {
            _seedingData.value = apiService.getSeeding(event.id, roundId)
            _error.value = null
        } catch (e: Exception) {
            _seedingData.value = null
...
fun SeedingItem(
    ...
    ListItem(
        headlineContent = {
            Text(
                seeding.name + formatRank(seeding.rating, seeding.fenRank)
            )
        },
        leadingContent = {
            Text(seeding.seed)
        ...
        supportingContent = {
            Text(
                listOfNotNull(
                    listOfNotNull(seeding.club1, seeding.club2).joinToString(" / "),
                    seeding.div,
                    seeding.country
                )
                    .joinToString(" - ")
...
fun SeedingView(
    ...
    val viewModel: SeedingViewModel =
        koinViewModel(parameters = { parametersOf(event, roundId) })

    val seedingData = viewModel.seedingData.collectAsStateWithLifecycle()
    ...
    PullToRefresh(
        ...
        Scaffold(modifier = Modifier.nestedScroll(scrollBehavior.nestedScrollConnection), topBar = {
            ...
            MediumFlexibleTopAppBar(
                title = {
                    Text(
                        seedingData.value?.title ?: "Seeding",
                    )
                },
                navigationIcon = {
                ...
                actions = {
                    IconButton(onClick = {
                        openLink(
                            "https://fencingtimelive.com/rounds/seeding/${event.id}/$roundId",
                            context
                        )
                    }) {
...
//                        searchBarState.collapsedCoords = coordinates
            ...
            LazyColumn(
                ...
                itemsIndexed(
                    seedingData.value?.seedings ?: emptyList(),
                    key = { index, seeding -> seeding.id }) { index, seeding ->
                    SeedingItem(
                        seeding = seeding,
                        modifier = Modifier.animateItem()
                    )
                    if (index != seedingData.value?.seedings?.lastIndex) {
                        HorizontalDivider()
                    }
                }
            }
            LoadingScreen(
                seedingData.value == null && error.value == null,
